...
//' @export
// [[Rcpp::export]]
 Rcpp::List decentralizedTraceQR_local_cpp(arma::mat &X, arma::vec &y,
                                     int d1, int d2,
                                     ...
                                     bool quiet = true)
 {
   ...
   arma::mat E(n, K);
   arma::uvec idx(n);
   arma::mat shat_mat(m, T_outer);//shat_mat(nlambda - 1, T_outer); //存储的是每次外部迭代计算每个机器上的估计
   arma::mat bic_mat(m, T_outer); //bic_mat(nlambda - 1, T_outer);
   ...
   for (int v = 0; v < T_outer; v++)
   {
    ...
    arma::vec lambda_array(nlambda);
     ...
     //新建一个矩阵存储BIC在每个worker上的计算值
     arma::mat bicj(m, nlambda -1, arma::fill::zeros);
     arma::mat shatj(m, nlambda - 1, arma::fill::zeros); // nlambda*m
     ...
     for (int ilambda = 0; ilambda < lambda_array.n_elem; ilambda++)
     {
       ...
       for (int j = 0; j < m; j++)
       {
         ...
         // bic_array(ilambda) = quantile_lossCPP(y(idx) - X.rows(idx) * B_inner.col(j), tau);
         shat = int(arma::rank(arma::reshape(B_inner.col(j), d1, d2)));
         shatj(j, ilambda) = shat;
         bicj(j, ilambda) = quantile_lossCPP(y(idx) - X.rows(idx) * B_inner.col(j), tau) / N + std::log(N) * std::log(std::log(N)) / N * std::max(shat, 1);
       }
       // shat = 1.0 * shat / m; // shat是记录全局rank平均值
     ...
     // if (!quiet) {
     //   Rcpp::Rcout << arma::trans()  << std::endl;
     // }



     arma::vec bicj_min = arma::min(bicj, 1); //对应每个worker上最小的bic
     arma::uvec lambda_index_min = arma::index_min(bicj, 1); //最小bic的index

     if (!quiet)
     {
       Rcpp::Rcout << bicj_min << std::endl;
     }


     //shat_mat.col(v) = bicj_min;//shat_array; // 这个要放到循环内部写才能记录

     bic_mat.col(v) = bicj_min; //bic_array;  // bic_mat记录是每次外层迭代对应的每个worker最小的bic
     ...
     for (int t = 0; t < T_inner; t++)
     {
       ...
       for (int j = 0; j < m; j++)
       {
         int ilambdamin = lambda_index_min(j);
         lambda = lambda_array(ilambdamin);  //选BIC最小的那个lambda
         int shat_j = shatj(j, ilambdamin);
         shat_mat(j,v) =  shat_j;
